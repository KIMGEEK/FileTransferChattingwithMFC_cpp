...
BEGIN_MESSAGE_MAP(Cipc2023Dlg, CDialogEx)
	...
	ON_CBN_SELCHANGE(IDC_COMBO_ADAPTER_LIST, &Cipc2023Dlg::OnCbnSelchangeComboAdapterList) //콤보박스에서 어댑터 선택마다 호출되는 함수를 처리하도록 함
END_MESSAGE_MAP()
...
BOOL Cipc2023Dlg::Receive(unsigned char* ppayload)
{
	...
	// 이더넷 헤더에서 출발지와 목적지 주소 추출
	UCHAR* srcMac = m_Ethernet->GetSourceAddress();
	UCHAR* dstMac = m_Ethernet->GetDestinAddress();

	CString srcAddr, dstAddr;
	UctoS(srcMac, srcAddr);
	UctoS(dstMac, dstAddr);

	// 메시지 형식 만들기
	CString formattedMsg;
	formattedMsg.Format(_T("[%s -> %s] %s"), dstAddr, srcAddr, CString((LPCTSTR)ppayload));

	m_ListChat.AddString(formattedMsg);
	return TRUE;
}
...
LRESULT Cipc2023Dlg::OnRegSendMsg(WPARAM wParam, LPARAM IParam)
{
	if (m_nAckReady) {
		if (m_LayerMgr.GetLayer("Network")->Receive())
		{
...
{	
	...
	{
		AfxMessageBox(_T("주소를 설정 오류 발생", "경고"),
			MB_OK | MB_ICONSTOP);
